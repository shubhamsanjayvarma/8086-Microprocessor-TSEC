...
import { 
  FileCode, Terminal, 
  AlertCircle, Info, HelpCircle, Sun, Moon,
  Download, Copy, Check, Accessibility
} from 'lucide-react';
...
export default function App() {
  ...
  const fmtHex = (val: number, digits: 2 | 4 = 4) => {
    return (val & (digits === 2 ? 0xFF : 0xFFFF)).toString(16).padStart(digits, '0').toUpperCase();
  };
  ...
  return (
    <div className={`yj-app-wrapper ${isDarkMode ? 'dark-theme' : 'light-theme'}`}>
      
      {/* TOP HEADER NAVBAR */}
      <header className="yj-navbar">
        <div className="yj-navbar-left">
          <span className="yj-logo-text" onClick={() => setViewMode(viewMode === 'compiler' ? 'landing' : 'compiler')}>
            8086 Compiler
          </span>
        </div>
        ...
        <div className="yj-landing-container">
          <section className="yj-hero-card">
            ...
            <div className="yj-hero-text-col">
              ...
              <div className="yj-hero-btns">
                <button className="yj-btn-gold" onClick={() => setViewMode('compiler')}>
                  TRY ONLINE 8086 COMPILER
                </button>
                <button className="yj-btn-outlined" onClick={() => {
          ...
          <div className="yj-features-grid">
            <div className="yj-feature-box">
              ...
              <p>We know that programmers love the dark theme. Toggle between Bright and Dark Theme using the theme button in Navbar.</p>
            </div>
            <div className="yj-feature-box">
              <h3>Access To 1 MB Memory</h3>
              <p>The Emulator supports complete 1 MB Memory, which can be accessed from the memory table.</p>
            </div>
            <div className="yj-feature-box">
              <h3>Validated Jump and Call using Labels</h3>
              <p>As Jumps and calls only allows valid labels, it does not permit any jumps to incorrect position.</p>
            </div>
            <div className="yj-feature-box">
              <h3>Selected Interrupts</h3>
              <p>Being an Emulator, this does not have 'true' memory so it supports select few interrupts.</p>
            </div>
            <div className="yj-feature-box">
              <h3>Line by Line Execution</h3>
              <p>Supports running all instructions automatically, or manual line by line execution. You can also stop the automatic execution with a simple button click.</p>
            </div>
            <div className="yj-feature-box">
              <h3>Check Registers and Flags in Real Time</h3>
              <p>Updates Flags and registers along with the execution, so can check the state of Emulator easily, all in a single view.</p>
            </div>
          </div>
            ...
            <div className="yj-col-left">
              ...
              <div className="yj-editor-box">
                ...
                <div className="yj-editor-side-tools">
                  <button className="yj-side-icon-btn" onClick={handleDownloadAsm} title="Download .asm File">
                    <Download size={18} />
                  </button>
                  <button className="yj-side-icon-btn" onClick={handleNextSample} title="Load Next Sample Code">
                    <FileCode size={18} />
                  </button>
                  <button className="yj-side-icon-btn" onClick={handleCopyCode} title="Copy Code">
                    {copied ? <Check size={18} color="#228b22" /> : <Copy size={18} />}
                  </button>
                </div>
              </div>
              ...
              <div className="yj-input-section">
                ...
                <div className="yj-input-box-wrapper">
                  ...
                  />
                  <button className="yj-input-submit-btn" onClick={handleInputSubmit} title="Submit Input">
                    ✓
                  </button>
                </div>
              </div>
              ...
              <div className="yj-output-section">
                ...
                <div className="yj-output-box">
                  {cpuState.consoleOutput ? (
                    <pre className="yj-output-text">{cpuState.consoleOutput}</pre>
                  ) : (
                    <span className="text-dim italic">Runtime output will appear here...</span>
                  )}
                </div>
              </div>
                ...
                <div className="yj-card-table">
                  ...
                  <div className="yj-card-table-body">
                    <div className="yj-table-row">
                      ...
                      <span className="yj-cell-val">{fmtHex((cpuState.registers.AX >> 8) & 0xFF, 2)}</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.AX & 0xFF, 2)}</span>
                    </div>
                    <div className="yj-table-row">
                      <span className="yj-cell-lbl">B</span>
                      <span className="yj-cell-val">{fmtHex((cpuState.registers.BX >> 8) & 0xFF, 2)}</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.BX & 0xFF, 2)}</span>
                    </div>
                    <div className="yj-table-row">
                      <span className="yj-cell-lbl">C</span>
                      <span className="yj-cell-val">{fmtHex((cpuState.registers.CX >> 8) & 0xFF, 2)}</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.CX & 0xFF, 2)}</span>
                    </div>
                    <div className="yj-table-row">
                      <span className="yj-cell-lbl">D</span>
                      <span className="yj-cell-val">{fmtHex((cpuState.registers.DX >> 8) & 0xFF, 2)}</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.DX & 0xFF, 2)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="yj-card-table">
                  ...
                  <div className="yj-card-table-body">
                    <div className="yj-table-row-2col">
                      ...
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.SS, 4)}</span>
                    </div>
                    <div className="yj-table-row-2col">
                      <span className="yj-cell-lbl">DS</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.DS, 4)}</span>
                    </div>
                    <div className="yj-table-row-2col">
                      <span className="yj-cell-lbl">ES</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.ES, 4)}</span>
                    </div>
                    <div className="yj-table-row-2col">
                      <span className="yj-cell-lbl">CS</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.CS, 4)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="yj-card-table">
                  ...
                  <div className="yj-card-table-body">
                    <div className="yj-table-row-2col">
                      ...
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.SP, 4)}</span>
                    </div>
                    <div className="yj-table-row-2col">
                      <span className="yj-cell-lbl">BP</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.BP, 4)}</span>
                    </div>
                    <div className="yj-table-row-2col">
                      <span className="yj-cell-lbl">SI</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.SI, 4)}</span>
                    </div>
                    <div className="yj-table-row-2col">
                      <span className="yj-cell-lbl">DI</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.DI, 4)}</span>
                    </div>
                    <div className="yj-table-row-2col yj-ip-row">
                      <span className="yj-cell-lbl">IP</span>
                      <span className="yj-cell-val">{fmtHex(cpuState.registers.IP, 4)}</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="yj-flags-card">
                ...
                <div className="yj-flags-table">
                  ...
                  <div className="yj-flags-val-row">
                    <span className={cpuState.flags.OF ? 'flag-on' : ''}>{cpuState.flags.OF ? 1 : 0}</span>
                    <span className={cpuState.flags.DF ? 'flag-on' : ''}>{cpuState.flags.DF ? 1 : 0}</span>
                    <span className={cpuState.flags.IF ? 'flag-on' : ''}>{cpuState.flags.IF ? 1 : 0}</span>
                    <span className="flag-off">0</span>
                    <span className={cpuState.flags.SF ? 'flag-on' : ''}>{cpuState.flags.SF ? 1 : 0}</span>
                    <span className={cpuState.flags.ZF ? 'flag-on' : ''}>{cpuState.flags.ZF ? 1 : 0}</span>
                    <span className={cpuState.flags.AF ? 'flag-on' : ''}>{cpuState.flags.AF ? 1 : 0}</span>
                    <span className={cpuState.flags.PF ? 'flag-on' : ''}>{cpuState.flags.PF ? 1 : 0}</span>
                    <span className={cpuState.flags.CF ? 'flag-on' : ''}>{cpuState.flags.CF ? 1 : 0}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="yj-memory-header-line">
                  ...
                  <div className="yj-start-addr-wrapper">
                    ...
                    <input 
                      ...
                      onChange={e => setMemStartAddressHex(e.target.value.replace(/[^0-9a-fA-F]/g, '').slice(0, 5))}
                      maxLength={5}
                    />
                    <button 
                      className="yj-btn-set"
                      onClick={() => setMemStartAddressHex(memStartAddressHex.padStart(5, '0'))}
                    >
                      SET
                    </button>
                  </div>
